...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if pagina == PAGINAS_TES[0]:
        ...
        # ── FORA DO FORM: Docente + lookup (actualiza em tempo real) ─────────
        # CORRECÇÃO DO BUG: o st.selectbox do docente DEVE ficar fora do form.
        # Dentro de um form, o Streamlit só lê o valor ao submeter — até lá
        # devolve sempre o primeiro item da lista (Manuel Octávio).
        st.markdown("**Selecção do Docente**")
        col_doc, col_ext = st.columns([3, 2])
        with col_doc:
            docente_sel_tes = st.selectbox(
                "Docente *",
                MEMBER_NAMES + ["[Externo]"],
                key="sel_doc_tes",
            )

        # Externo — campos também fora do form para leitura imediata
        if docente_sel_tes == "[Externo]":
            with col_ext:
                nome_ext  = st.text_input("Nome do docente externo *", key="ext_nome_tes")
                grau_ext  = st.selectbox("Grau", ["PhD","MSc","Lic"], key="ext_grau_tes")
                cargo_ext = st.text_input("Cargo / Instituição", key="ext_cargo_tes")
            mem_sel_tes   = {"Nome": nome_ext, "Grau": grau_ext,
                             "Cargo": cargo_ext, "Departamento": "Externo"}
            docente_final = nome_ext.strip()
        else:
            mem_sel_tes   = get_member(docente_sel_tes)
            docente_final = docente_sel_tes

        grau_sel  = mem_sel_tes.get("Grau",  "MSc")
        cargo_sel = mem_sel_tes.get("Cargo", "Docente")

        # Info actualizada em tempo real (fora do form = correcto)
        st.info(f"**Nível:** {grau_sel}  |  **Cargo:** {cargo_sel}")

        # Modalidade também fora do form — condiciona as funções disponíveis
        modalidade = st.selectbox(
            "Modalidade *",
            ["Defesa TFC","Pré-Defesa TFC","Relatório de Estágio"],
            key="sel_modalidade_tes",
        ...
        # ── DENTRO DO FORM: apenas campos que são processados na submissão ──
        with st.form("form_lancamento", clear_on_submit=False):
            ...
            with c2:
                # Mostra o docente já seleccionado (só leitura dentro do form)
                st.markdown(f"**Docente:** {docente_final or '—'}")
        ...
        if submitted:
            errors = []
            if not docente_final:      errors.append("Nome do docente obrigatório")
            ...
            if docente_final and check_duplicate(despacho.strip(), docente_final):
                errors.append(
                    f"Erro: Registo duplicado — {docente_final} já consta no Despacho {despacho}")
            ...
            if errors:
                ...
                reg = {
                    "despacho": despacho.strip(), "docente": docente_final,
